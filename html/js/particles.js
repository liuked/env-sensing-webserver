...
function init() {
  ...
  var pmsAddress = scheme + window.location.host + '/sensors/pms';
  ...
  return [ r * 255, g * 255, b * 255 ];
	...
	function onTimer() {
		...
		count++;
			...
			h=h+1;
			//particle color
			[r,g,b] = hsvToRgb((h)%360/360,0.7,1)
			
			rgbString = "rgba("+r+","+g+","+b+","; //partial string for color which will be completed by appending alph
			n=n+0.02;
			numToAddEachFrame = Math.floor(n)%20
			pm1 = Math.floor(Math.random()*5);
			pm25 = Math.floor(pm1+ Math.random()*10);
			pm10 = Math.floor(pm25+ Math.random()*5);
			document.getElementById("disppm1").innerHTML = pm1;
			document.getElementById("disppm25").innerHTML = pm25;
			document.getElementById("disppm10").innerHTML = pm10;
